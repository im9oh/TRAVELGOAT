...
export default function Places() {
  ...
  const filtered = useMemo(() => {
    const list =
      filterCity === 'all' ? places : places.filter((p) => (p.cityId ?? 'none') === filterCity)
  ...
  return (
    <div>
      <SectionTitle
        ...
        action={<Button size="sm" variant="purple" onClick={openNew}>+ Place</Button>}
      />
        ...
        <Card className="mb-4 flex items-center gap-5 bg-gradient-to-b from-[#faf3ff] to-white">
          ...
          <div className="flex-1">
            <div className="text-lg font-extrabold text-[#3c3c3c]">
              {places.length - visitedCount} to explore
            </div>
            <p className="text-sm font-bold text-[#afafaf]">
              Tick places off as you go — each is worth 25 XP!
            </p>
          </div>
        </Card>
      )}
      ...
      {filtered.length === 0 ? (
        <EmptyState icon="📍" title="No places yet" hint="Save sights, food, and activities you don't want to miss." />
      ) : (
            ...
            <Card key={p.id} className={`!p-3 ${p.visited ? 'opacity-70' : ''}`}>
              <div className="flex items-start gap-3">
                <button
                  ...
                  className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-sm font-black transition ${
                    p.visited
                ...
                <div className="min-w-0 flex-1" onClick={() => openEdit(p)}>
                  <div className={`font-extrabold text-[#3c3c3c] ${p.visited ? 'line-through' : ''}`}>
                  ...
                  {p.url && (
                    <a
                      ...
                      className="mt-1 inline-block text-sm font-extrabold text-[#1cb0f6]"
                    >
                      Open link ↗
                    </a>
                  )}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Category">
                <Select
                  ...
                  onChange={(e) => setEditing({ ...editing, category: e.target.value as PlaceCategory })}
                >
                  {CATEGORIES.map((c) => (
